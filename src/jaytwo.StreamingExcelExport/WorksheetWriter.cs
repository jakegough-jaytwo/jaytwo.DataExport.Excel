...
public class WorksheetWriter<T> : XmlDocumentWriter
{
    ...
    public WorksheetWriter(XmlWriter writer)
        : base(writer)
    {
    ...
    public async Task WriteAsync(IAsyncEnumerable<T> data, CancellationToken cancellationToken)
    {
        ...
        await using (CreateElementScope("worksheet", "http://schemas.openxmlformats.org/spreadsheetml/2006/main"))
        {
            ...
            {
                int rowNumber = 1;
                await foreach (var itemValues in GetCellData(data, writeHeader: true).WithCancellation(cancellationToken))
                {
